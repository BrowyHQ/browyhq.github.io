...
**Yes.** Open the extension Settings → Tools. Every browser tool is on by default and can be toggled off; the disabled ones are stripped from the prompt the model sees. Host tools (`bash`, `read_file`, `write_file`, `grep`, `glob`, `web_fetch`) are off by default and can be opted into per-tool. See [Tools](/tools/).
