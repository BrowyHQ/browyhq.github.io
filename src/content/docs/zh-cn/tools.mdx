---
title: Browy 智能体可以调用的工具
description: Browy 暴露给大模型的每一个浏览器工具，按检查、点击和输入、标签页、页面脚本、网络与控制台、磁盘与记忆分组，以及可选开启的主机工具（shell、文件系统、web fetch）。
---
...
| `extract_form` | 列出表单字段，给 `fill_form` 用。先读一次结构，再一次调用填完。[这对工具是怎么配合的](/zh-cn/blog/filling-forms-signed-in/)。 |
