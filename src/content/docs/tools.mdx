...
description: Every tool Browy exposes to the LLM, plus the host tools you can opt into.
---
...
`glob`, and `web_fetch` as standard tools — the same ones the `copilot`
CLI uses for coding work in a terminal. They reach your machine's
filesystem and shell, so Browy keeps them off by default.

To enable one, open Settings → Tools and scroll to the **host
(advanced)** group. Each tool is a separate toggle. The native host
keeps an allowlist of accepted names, so a compromised extension
can't quietly enable anything outside this set.

If you need a terminal coding agent without the browser side at all,
use the [GitHub Copilot CLI](https://docs.github.com/en/copilot/github-copilot-in-the-cli)
directly.
