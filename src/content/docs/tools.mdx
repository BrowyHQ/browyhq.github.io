---
title: Tools the Browy agent can call from chat
description: Every browser tool Browy exposes to the LLM, grouped by inspect, click and type, tabs, page scripting, network and console, disk and memory, plus the opt-in host tools (shell, filesystem, web fetch).
---

import { CardGrid, Card, LinkCard, Aside, Badge, Tabs, TabItem } from '@astrojs/starlight/components';

Browy ships with the browser-driving tools on. Turn any of them off in
**Settings → Tools** if you want a stricter setup (read-only, no JS
evaluation, and so on).

The Copilot SDK also exposes a set of **host tools** (`bash`,
`read_file`, `write_file`, `grep`, `glob`, `web_fetch`) that reach your
machine's filesystem and shell. These are off by default. Turn them on
one at a time in Settings.
...
|---|---|---|---|
| **Browser tools** (everything below) | ON | The active tab via CDP | Turn off individually in Settings |
| **Host tools** (`bash`, `read_file`, `write_file`, `grep`, `glob`, `web_fetch`) | OFF | Your machine, with your user privileges | Turn on individually in Settings → Tools → host (advanced) |
...
| `extract_form` | Enumerate inputs of a form for `fill_form`. Read the schema once, then fill in one call. [How the pair works together](/blog/filling-forms-signed-in/). |
...
grouped by category: _inspect_, _navigate_, _interact_, and _devtools_.
...
If you only need a terminal coding agent, the
...
<CardGrid>
  <LinkCard title="Permissions" href="/permissions/" description="Why the extension asks for the Chrome permissions it does." />
  <LinkCard title="Privacy & data" href="/privacy/" description="What stays on your machine and what goes to GitHub Copilot." />
