---
title: "Architecture: Browy native messaging host (backend)"
description: How the Browy native messaging host wraps the GitHub Copilot SDK, dispatches tools over the native messaging port to the extension, manages per-port sessions, and writes its rotating diagnostic log.
---
...
[Settings → Tools](/tools/#configurable-per-tool-toggles) toggles.
...
<CardGrid>
  <LinkCard title="Architecture: extension (frontend)" href="/arch-frontend/" description="Service worker, side panel, DevTools panel, content scripts." />
  <LinkCard title="Architecture: DevTools CLI" href="/arch-cli/" description="Slash commands, completion, model picker, REPL state." />
  <LinkCard title="Tools the agent has" href="/tools/" description="Full reference of each tool, plus the per-tool Settings toggles." />
</CardGrid>
